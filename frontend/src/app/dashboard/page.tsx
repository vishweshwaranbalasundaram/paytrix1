...
export default function PaytrixAppleDashboard() {
  ...
  const AGENT_HEADERS = {
    ...
    "X-Agent-Id": process.env.NEXT_PUBLIC_AGENT_ID || "agt_demo_ui",
    "X-Agent-Key": process.env.NEXT_PUBLIC_AGENT_KEY || "demo_agent_secret_key_do_not_use_in_prod",
  };

  const makePayload = (suffix: string): CheckoutPayload => ({
    user_id: "usr_demo_123",
    user_prompt: "Order monthly diabetes care supplies under \u20b91,500",
    trace_id: `tr_apple_${Date.now()}_${suffix}`,
    intent_envelope: {
  ...
  const runSimulation = async (scenario: "happy" | "dark_pattern" | "scalping" | "velocity") => {
    setLoading(true);
    try {
      if (scenario === "velocity") {
        // Fire 4 rapid happy-path requests — the 4th should trip the
        // Velocity Governor (max 3 debits / 10 min) even though every
        // individual request is otherwise perfectly valid.
        let data: any = null;
        for (let i = 0; i < 4; i++) {
          const payload = makePayload(`vel${i}`);
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-slate-950/60 border-b border-white/10 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/"
              className="text-xs font-mono px-3 py-1.5 rounded-full bg-slate-900/80 border border-white/10 text-slate-300 hover:text-cyan-300 hover:border-cyan-500/30 transition-colors"
            >
              ← Landing
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="p-6 rounded-3xl bg-slate-900/40 border border-white/10 backdrop-blur-2xl space-y-6">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => runSimulation("velocity")}
                disabled={loading}
                className="w-full p-4 rounded-2xl bg-violet-950/20 hover:bg-violet-950/30 border border-violet-500/30 text-left flex items-center justify-between group transition-all disabled:opacity-50"
              >
                <div>
                  <div className="font-medium text-sm text-violet-200">
                    Simulate Velocity Breach
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    Fires 4 rapid debits &mdash; 4th is blocked by the rolling-window cap
                  </div>
                </div>
                <Activity className="w-5 h-5 text-violet-400 group-hover:scale-110 transition-transform" />
              </button>
            </div>
          </section>
          ...
          <section className="p-6 rounded-3xl bg-slate-900/40 border border-white/10 backdrop-blur-2xl space-y-4">
            ...
            {lastResponse ? (
              <div className="space-y-4">
                ...
                {lastResponse.alignment_breakdown && (
                  <div className="p-4 rounded-2xl bg-slate-950/80 border border-cyan-500/20 space-y-2">
                    <div className="text-xs font-mono text-cyan-400 mb-1">
                      I2A Score Breakdown &mdash; {(lastResponse.alignment_breakdown.score * 100).toFixed(1)}%
                    </div>
                    {[
                      ["Price Fit (40%)", lastResponse.alignment_breakdown.price_component],
                      ["Trust (35%)", lastResponse.alignment_breakdown.trust_component],
                      ["Rating (25%)", lastResponse.alignment_breakdown.rating_component],
                    ].map(([label, value]: any) => (
                      <div key={label} className="flex items-center justify-between text-[11px] font-mono text-slate-400">
                        <span>{label}</span>
                        <div className="flex items-center gap-2 flex-1 mx-3">
                          <div className="h-1.5 flex-1 rounded-full bg-white/5 overflow-hidden">
                            <div
                              className="h-full bg-cyan-500/60 rounded-full"
                              style={{ width: `${Math.min(100, (value / 0.4) * 100)}%` }}
                            />
                          </div>
                        </div>
                        <span className="text-slate-300">{value.toFixed(3)}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {/* Risk flags */}
                {lastResponse.risk_flags && lastResponse.risk_flags.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {lastResponse.risk_flags.map((flag: string) => (
                      <span
                        key={flag}
                        className="text-[10px] font-mono px-2 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20"
                      >
                        ⚑ {flag}
                      </span>
                    ))}
                ...
                {lastResponse.idempotent_replay && (
                  <div className="text-[10px] font-mono px-2 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 inline-block">
                    ↺ Idempotent replay &mdash; cached response, kernel not re-run
                  </div>
                )}
